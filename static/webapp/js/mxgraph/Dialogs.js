...
var EditDataDialog = function(ui, cell)
{
	...
	{
		names[index] = name;
		texts[index] = form.addTextarea(names[count] + ':', value, 4);
	...
	// if(attrs.length == 0){
	//   temp.push({name: mxResources.get('deviceid'), value: ''});	
	// }
	...
	if(temp.length == 0){ //object 属性长度为 0 里 加入 设备id
		addTextArea(0, mxResources.get('deviceid'), '');	
	}
	for (var i = 0; i < temp.length; i++)
	{
		// console.log(temp[i].name, 'temp[i].name')
		if(temp[i].name != 'modelTypeId' && temp[i].name != 'modelTypeLaberId'){ //如果存在 "modelTypeId" 不允许编辑
			// console.log(temp[i].name, 'dd')
	...
	nameInput.setAttribute('size', (mxClient.IS_IE || mxClient.IS_IE11) ? '51' : '51');
		...
		if (name.length > 0 && name != 'label' && name != 'placeholders' && name.indexOf(':') < 0)
		{
			try
			{
				...
				else
				{
					...
					names.push(name);
					var text = form.addTextarea(name + ':', '', 4);
	...
	addBtn.style.verticalAlign = 'top';
