...
(function()
{
	...
	Menus.prototype.init = function()
	{
		...
		editorUi.actions.put('exportXml', new Action(mxResources.get('formatXml') + '...', function()
		{
			...
			//导出 时， 是否选择压缩
			var pages = editorUi.addCheckbox(div, mxResources.get((noPages) ? 'compressed' : 'allPages'), false);
	 ...
	 //                }
		...
		{
			this.addMenuItems(menu, ['undo', 'redo', '-', 'cut', 'copy', 'paste', 'delete', '-', 'duplicate', '-', 'find', '-', 'editData', 'editTooltip', '-', 'editStyle', 'editGeometry', '-', 'edit', '-', 'editLink', 'openLink', '-', 'selectVertices', 'selectEdges', 'selectAll', 'selectNone', '-', 'lockUnlock']);
		})));
		...
		this.put('file', new Menu(mxUtils.bind(this, function(menu, parent)
		{
			...
			else
			{
				...
				if (file != null && file.constructor == DriveFile)
				{
					this.addMenuItems(menu, ['new', '-', 'rename'], parent); //'makeCopy', , 'moveToFolder'
				}
				else
				{
					if (!mxClient.IS_CHROMEAPP && !EditorUi.isElectronApp &&
					...
					if(EditorUi.prototype.getUrlParam('currentId')==""){
