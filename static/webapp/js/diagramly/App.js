 ...
 */
function App (editor, container, lightbox)
{
...
App.GET_CURENTFILE_XML_DATA = null;

App.GET_CURENTFILE_XML_NAME = null;

App.GET_CURRENTFILE_XML_ID = EditorUi.prototype.getUrlParam('currentId');

App.GET_CURRENTFILE_CELL_ID = EditorUi.prototype.getUrlParam('cellId');

App.GET_CURRENTFILE_ADD_TYPE = EditorUi.prototype.getUrlParam('addType');

App.GET_BASE_API_URL = 'http://soc.122.204.161.218.xip.io:9023/api/'
/**
 ...
 * Sets the delay for autosave in milliseconds. Default is 2000.
 */
App.DROPINS_URL = 'js/dropins/dropins.js';//'https://www.dropbox.com/static/api/2/dropins.js';
 ...
 */
App.ONEDRIVE_URL = mxClient.IS_IE11? 'js/onedrive/OneDrive.js' : 'js/onedrive/OneDrive.js';

/**
 * Trello URL
 */
App.TRELLO_URL = 'js/client/client.js'; // 'https://api.trello.com/1/client.js';

/**
 * Trello JQuery dependency
 */
App.TRELLO_JQUERY_URL = '/js/jquery.v3.2.1.min.js';
 ...
 */
App.PUSHER_URL ='js/pusher/pusher.min.js' ;//'https://js.pusher.com/4.3/pusher.min.js';
...
// App.getUrlParam = function(paraName) {
...
App.getCurrentXml = function(){
  if(App.GET_CURRENTFILE_ADD_TYPE == 'PRIMITIVE'){
  	var url = App.GET_BASE_API_URL + 'mx_graph/mx_primitive/' + App.GET_CURRENTFILE_XML_ID + '/';
  }else{	
  	var url = App.GET_BASE_API_URL + 'mx/' + App.GET_CURRENTFILE_XML_ID + '/';
  }
  console.log(url, 'url')
	...
	request.onreadystatechange = function () { // 状态发生变化时，函数被回调
	    if (request.readyState === 4) { // 成功完成
	        ...
	        if (request.status === 200) {
        	  ...
        	  App.GET_CURENTFILE_XML_DATA = responseData.xml.replace(/&amp;/g, '&');
        	  App.GET_CURENTFILE_XML_NAME = responseData.name;
        	  console.log(responseData, 'readyState')
	            // 成功，通过responseText拿到响应的文本:
	            // this.editorUi.alert(mxResources.get('saveSuccess'))
	            // return request.responseText;
	        } else {
	            // 失败，根据响应码判断失败原因:
	            this.editorUi.alert(mxResources.get('fail'))
			...
			if (urlParams['embed'] != '1')
			{
				if (typeof window.DriveClient === 'function')
				{
						...
						(document.documentMode == null || document.documentMode >= 10))
					{
						...
						{
							mxscript('js/googleApi/api.js'); //https://apis.google.com/js/api.js
						}
...
App.main = function(callback, createUi)
{
	...
	if (window.mxscript != null)
	{
		...
		if (urlParams['plugins'] != '0' && urlParams['offline'] != '1')
		{
			...
			// //MXSettings尚未在配置模式下初始化，重定向参数
			//对于嵌入模式的插件，调用方中的p url参数
			...
			if (plugins != null && plugins.length > 0 && urlParams['plugins'] != '0')
			{
				...
				//在下面的异步块中加载插件会阻止页面加载，因此
				//由于在下面加载了资源，因此使用了警告对话框的硬编码消息
		...
		// 负载/ GAPI全鸭browsers IE8下面，但如果不是残疾人或如果在embed模式化
		//特殊案例：不能在异步加载下面的代码
		if (typeof window.DriveClient === 'function' &&
	...
	function doLoad(bundle)
	{
		...
		//预取异步请求，以便下面的代码同步运行
		//通过mxresources中的回退系统加载正确的包（一个文件）。样式表
		//在构建过程中编译为JS，仅用于本地开发。
		mxUtils.getAll((urlParams['dev'] != '1') ? [bundle] : [bundle, (uiTheme == 'dark') ? STYLE_PATH + '/dark-default.xml' : STYLE_PATH + '/default.xml'], function(xhr)
		{
			...
			if (window.mxscript != null)
			{
					...
					isSvgBrowser && (document.documentMode == null || document.documentMode > 9))))
				{
					...
					{
						// Must load this after the dropbox SDK since they use the same namespace
						// 必须在Dropbox SDK之后加载此，因为它们使用相同的命名空间
				...
				// Loads OneDrive for all browsers but IE6/IOS if not disabled or if enabled and in embed mode
				// 如果未禁用或启用并处于嵌入模式，则为除IE6/IOS以外的所有浏览器加载OneDrive
				if (typeof window.OneDriveClient === 'function' &&
				...
				// Loads Trello for all browsers but < IE10 if not disabled or if enabled and in embed mode
				// 加载所有浏览器的trello，但如果未禁用或启用且处于嵌入模式，则小于ie10
				if (typeof window.TrelloClient === 'function' &&
	...
	function doMain()
	{
		...
		// //添加所需资源（禁止加载回退属性，这只能
		//如果我们知道所有键都是在特定语言的文件中定义的，则使用）
		mxResources.loadDefaultBundle = false;
		doLoad(mxResources.getDefaultBundle(RESOURCE_BASE, mxLanguage) ||
			mxResources.getSpecialBundle(RESOURCE_BASE, mxLanguage));
	};

	// console.log(App.GET_CURRENTFILE_XML_ID, 'ddd')
	if(App.GET_CURRENTFILE_XML_ID != '' && App.GET_CURRENTFILE_CELL_ID == ''){
	  App.getCurrentXml();
	}
	...
	{ 
		console.log('到哪儿去？')
		//xml model 组件列表数据
		// EditorUi.prototype.getXmlListTemplateData();
		
		if(App.GET_CURRENTFILE_XML_ID != null){
		  setTimeout(function(){
		  	doMain();
		  }, 2000)	
		}else{
 ...
 * 
 * Authorizes the client, gets the userId and calls <open>.
 * 授权客户端，获取用户ID并调用<open>。
 */
App.prototype.appIconClicked = function(evt)
{
	if (mxEvent.isAltDown(evt))
	{
		console.log(evt, 'appIconClicked')
...
App.prototype.open = function()
{
	...
	try
	{
		...
		// //如果在嵌入模式下使用create url参数，则
		//我们尝试从window.opener[value]打开XML。
		//将其用于嵌入via选项卡以绕过计时
		//传递没有OnLoad事件的消息时出现问题。
...
App.prototype.load = function()
{
	...
	if (urlParams['embed'] != '1')
	{
		...
		if (this.spinner.spin(document.body, mxResources.get('starting')))
		{
			try
			{
				...
				// console.log(this.stateArg, "this.stateArg")
			}
			catch (e)
			{
	...
	// 	{
	// 		console.log(evt, 'ent')
	// 		let isCreate = evt.data.createStatus
	// 		console.log(isCreate, 'isCreate')
	// 		if (isCreate == false){
	// 			var data = evt.data.xml;
...
App.prototype.start = function()
{
	...
	try
	{
		...
		{
			// console.log(this.getCurrentFile(), '123454')
				...
				value != null && value.length > 0 && this.spinner.spin(document.body, mxResources.get('loading')))
			{
				...
				{
					this.spinner.stop();
					// console.log(xml, 'showCreateDialog')
			...
			else
			{
					...
					this.drive != null && this.stateArg != null && this.stateArg.action == 'create')
				{
					...
					console.log(App.MODE_GOOGLE,'MODE_GOOGLE')
	...
	var showSecondDialog = mxUtils.bind(this, function()
	{
		...
		this.showDialog(dlg.container, 340, (serviceCount < 2 ||
			...
			mxUtils.bind(this, function(cancel)
			{
				...
				{
					// console.log(mxClient.IS_CHROMEAPP,'mxClient.IS_CHROMEAPP')
					var prev = Editor.useLocalStorage;
...
{
	var fileCurrent = this.getCurrentFile();
	// var xml = mxUtils.getXml(this.editor.getGraphXml(this.editor.graph.isSelectionEmpty()))
	var basename = this.getBaseFilename(false) + '.xml';
	// var file = '<?xml version="1.0" encoding="UTF-8"?>\n' +	mxUtils.getXml(this.editor.getGraphXml(this.editor.graph.isSelectionEmpty()));
	...
	if (fileCurrent != null)
	{
		...
		if (!forceDialog && basename != null && this.mode != null)
		{
			this.save(basename, done);
		}
		else
		{
			var filename = (basename != null) ? basename : this.defaultFilename;
			...
			var dlg = new CreateDialog(this, filename, mxUtils.bind(this, function(name, mode)
			{
				...
				{
					if ((prev == null && mode == App.MODE_DEVICE) || mode == 'save')
					{
						this.setMode(App.MODE_DEVICE);
						this.save(name, done);
					}
					...
					else if (mode == '_blank')
					{
						...
						window.openFile.setData(this.getFileData(false));
			...
			{
				this.hideDialog();
			}), mxResources.get('saveAs'), mxResources.get('download'), null, null, allowTab,(this.isOffline()) ? null :'',	true, rowLimit);
			this.showDialog(dlg.container, 460, (serviceCount > rowLimit) ? 390 : 140, true, true);
			dlg.init();
		}
	}
};
...
App.prototype.createFile = function(title, data, libs, mode, done, replace, folderId, tempFile)
{
	...
	if (title != null && this.spinner.spin(document.body, mxResources.get('inserting')))
	{
		...
		var error = mxUtils.bind(this, function(resp)
		{
			...
			{
				console.log(data, 'creatFile data')
				this.showSplash();
			}
			else if (resp != null)
			{
		...
		try
		{
			...
			{
				complete();
				console.log(data, 'creatFile data 2')
...
App.prototype.loadFile = function(id, sameWindow, file, success, force)
{
	...
	if (id == null || id.length == 0)
	{
		// console.log(App.GET_CURENTFILE_XML_DATA, 'App.GET_CURENTFILE_XML_DATA 1')
		if(App.GET_CURRENTFILE_XML_ID != null && App.GET_CURENTFILE_XML_DATA != null){
			this.fileLoaded(new LocalFile(this, App.GET_CURENTFILE_XML_DATA, App.GET_CURENTFILE_XML_NAME, false));
		}else{
		  fn();	
		}
	...
	{
		// console.log(App.GET_CURENTFILE_XML_DATA, 'App.GET_CURENTFILE_XML_DATA 2')
		if(App.GET_CURRENTFILE_XML_ID != null && App.GET_CURENTFILE_XML_DATA != null){
...
App.prototype.updateHeader = function()
{
	if (this.menubar != null)
	{
		...
		this.appIcon.style.backgroundColor = 'transparent';//'#f18808';
		...
		this.appIcon.style.backgroundSize = "100%";
