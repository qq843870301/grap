...
/**
 * [xmlModelDataList 模板数据]
 * @type {[type]}
 */
	EditorUi.xmlModelDataList = null;

	/**
 * [baseUrl 服务路由]
 * @type {String}
 */
	EditorUi.baseUrl_API = 'http://soc.122.204.161.218.xip.io:9023/api/';
	...
	EditorUi.prototype.createFileData = function(node, graph, file, url, forceXml, forceSvg, forceHtml, embeddedCallback, ignoreSelection, compact)
	{
		...
		else
		{
			...
			if (fileNode.nodeName.toLowerCase() != 'mxfile')
			{
				...
				// console.log(data,  'graph.compress(text);')
			...
			// console.log(xml, 'createFileData xml')
			return xml;
		}
	};
	...
	EditorUi.prototype.getXmlFileData = function(ignoreSelection, currentPage)
	{
		...
		{
			var data = this.editor.graph.zapGremlins(mxUtils.getXml(node)); //this.editor.graph.compressNode(node);
			...
			else
			{
				...
				for (var i = 0; i < this.pages.length; i++)
				{
					if (this.currentPage != this.pages[i] && this.pages[i].needsUpdate)
					{
						...
						mxUtils.setTextContent(this.pages[i].node, this.editor.graph.zapGremlins(mxUtils.getXml(temp))); //this.editor.graph.compressNode(temp)
	...
	EditorUi.prototype.getFileData = function(forceXml, forceSvg, forceHtml, embeddedCallback, ignoreSelection, currentPage, node, compact, file)
	{
		...
		// console.log(compact, 'getFileData compact')
		// console.log(result, 'getFileData result')
		return result;
	};
	...
	EditorUi.prototype.getHtml = function(node, graph, title, editLink, redirect, ignoreSelection)
	{
		ignoreSelection = (ignoreSelection != null) ? ignoreSelection : true;
		var bg = null;
		var js = '../embed-static.min.js';//'https://www.draw.io/js/embed-static.min.js';
	...
	{
		console.log(xml, getHtml2)
		console.log(redirect, 'redirect')
		var bg = null;
		var js = '../viewer.min.js';//'https://www.draw.io/js/viewer.min.js';
		var s = '';
	...
	EditorUi.prototype.createDownloadRequest = function(filename, format, ignoreSelection, base64, transparent, currentPage)
	{
      ...
      console.log(data, 'createDownloadRequest data')
	...
	EditorUi.prototype.fileLoaded = function(file)
	{
		...
		if (file != null)
		{
			try
			{
				...
				{
					var isModle = this.getUrlParam('addType'); //创建模式 图元模板 PRIMITIVE 图表 GRAPH
					if(isModle != 'PRIMITIVE'){
	...
	{
		console.log(this.isScratchpadEnabled(), 'isScratchpadEnabled')
		if (this.isScratchpadEnabled())
		{
			if (this.scratchpad == null)
			{
				...
				{
					this.getModelLastestData(1)
					console.log(xml, 'togg')
				}));
			}
			else
			{
	 ...
	 * 删除模板 xml 组件
	 */
EditorUi.prototype.deleteXmlDataModel = function(){
  // var url = EditorUi.baseUrl_API + 
}
	/**
	 * get mx xml model list datas
	 * xml 模板 列表数据
	*/
EditorUi.prototype.getXmlListTemplateData = function(url, fn){
		// var url = EditorUi.baseUrl_API + 'mx_graph/mx_primitive/'
	...
	/**
	 * post mx xml model list datas
	 * xml 模板 列表数据
	*/
	EditorUi.prototype.addXmlListTemplateData = function(params, fn){
		var url = EditorUi.baseUrl_API + 'mx_graph/mx_primitive/';
		var xhr = new XMLHttpRequest(); // 新建XMLHttpRequest对象
		xhr.onreadystatechange = function () { // 状态发生变化时，函数被回调
			  fn(xhr)
	...
	/**
	 * [getModelLastestData 获取图元模板数据]
	 * @param  {[type]}   status   [description]
	 * @param  {[type]}   file     [description]
	 * @param  {Function} callback [description]
	 * @return {[type]}            [description]
	 */
	EditorUi.prototype.getModelLastestData = function(status, file, callback){
		var url = EditorUi.baseUrl_API + 'mx_graph/mx_primitive/?group_id=all';
		var scr_xml = '';
		var scratchpadData = ''
		this.getXmlListTemplateData(url, mxUtils.bind(this, function(xhr){
			if (xhr.readyState == 4)
			{
				if (xhr.status >= 200 && xhr.status <= 299)
				{
					var xmlModelData = JSON.parse(xhr.response).results
					if(xmlModelData.length > 0){
						var xmlModelArr_scra = [];
						for(var i = 0; i < xmlModelData.length; i++){
							var xmlModeObject = {};
							xmlModeObject.title = xmlModelData[i].name;
							xmlModeObject.xml = this.editor.graph.compress(xmlModelData[i].xml);
							xmlModeObject.w = 80;
							xmlModeObject.h = 80;
							xmlModelArr_scra.push(xmlModeObject)
						}
						console.log(xmlModelArr_scra)
						scr_xml = this.createLibraryDataFromImages(xmlModelArr_scra);
						scratchpadData = new StorageLibrary(this, scr_xml, '.scratchpad')
						scratchpadData.graph_id = null
						this.loadLibrary(scratchpadData);
					}else{
						this.removeLocalData('.scratchpad');
					}
				}
				else
				{
					this.handleError({message: '获取图元模板失败'});
				}
			}
 ...
 /**
  * [xmlxy 修改xml(x,y)坐标]
  * @param  {Script} xmlString [description]
  * @return {Script}            [description]
  */
	EditorUi.prototype.xmlxy=function(xmlString) {
		let parser = new DOMParser();
		let xmlObject = parser.parseFromString(xmlString, "text/xml");
		let domElems = xmlObject.getElementsByTagName("mxGeometry");
		for (key of domElems) {
 ...
 /**
  * [getModeGroupListData 获取图元模板组数据]
  * @param  {Function} callback [description]
  * @return {[type]}            [description]
  */
	EditorUi.prototype.getModeGroupListData = function(callback){
		var url = EditorUi.baseUrl_API + 'mx_graph/primitive_group/'
		this.getXmlListTemplateData(url, mxUtils.bind(this, function(xhr){
			if (xhr.readyState == 4){
				if (xhr.status >= 200 && xhr.status <= 299){
					var groupModelData = JSON.parse(xhr.response).results
					if(groupModelData.length > 0){
						var group_model = []
						for(var i = 0; i < groupModelData.length; i++){
							var xmlModelArr = [];
							var group_model_name = groupModelData[i].name
							for(var a = 0; a < groupModelData[i].primitives.length; a++){
								var xmlModeObject = {};
								var xml=groupModelData[i].primitives[a].xml
								xmlModeObject.title = groupModelData[i].primitives[a].name;
								xmlModeObject.xml = this.editor.graph.compress(this.xmlxy(xml));
								xmlModeObject.w = 80;
								xmlModeObject.h = 80;
								xmlModelArr.push(xmlModeObject)	
							}
							var group_xml = this.createLibraryDataFromImages(xmlModelArr);
							// console.log(group_xml, 'group_xml')
							var files = new StorageLibrary(this, group_xml, group_model_name);
							files.group_id = groupModelData[i].id
							// console.log(files, 'files-----fiels')
							this.loadLibrary(files);
							// this.loadLibrary(new StorageLibrary(this, xmlModelArr, responseText.group_name));	
							// this.saveLibrary(group_model_name, xml, file, App.MODE_DEVICE);
						}
					}else{
						this.removeLocalData('.scratchpad');
					}
	...
	EditorUi.prototype.libraryLoaded = function(file, images, optionalTitle)
	{
	 ...
	 // console.log(images, 'images1')
		...
		// console.log(images, 'xmlModelDataList')
	    ...
	    {
	    	// console.log(file, 'getLibraryStorageHint scratchpad')
	    ...
	    buttons.style.zIndex = '5';
	    
	    // Workaround for CSS error in IE8 (standards and quirks)
	    if (!mxClient.IS_QUIRKS && document.documentMode != 8)
	    {
	    		buttons.style.backgroundColor = '#fff';//'inherit';
	    }
		...
		if (file.isEditable())
		{
			...
			{
				// console.log(file.getTitle(), 'editLibrary' )
			...
			{
				var cells = graph.cloneCells(mxUtils.sortCells(graph.model.getTopmostCells(cells)));
				var filename = this.getBaseFilename();
				var creator = this.getUrlParam('un')
				//加入模板 model 前数据校验
				var modelCell = mxUtils.getXml(this.editor.graph.encodeCells(cells));
				if(modelCell.indexOf('<object') < 0){
					this.handleError({message: '模板内容不合规，请绑定模板属性数据后继续提交！'})	
					// this.alert('模板内容不合规，请绑定模板属性数据后继续提交！')
					return
				}else{
					if(filename == '' || filename == '未命名表单'){
						this.handleError({message:'模板名称不合规，请添加模板名称后继续提交！'})	
				...
				if(bounds.group_id=='undefined'){
					var cellParams={
						name: filename, //模板名称
						creator: creator, //用户ID
						xml: modelCell //当前图元模板信息
					}
				}else{
					var cellParams={
						name: filename, //模板名称
						group_id: bounds.group_id, //图元组模板ID
						creator: creator, //用户ID
						xml: modelCell //当前图元模板信息
					}
				}
				
				// console.log(filename, 'filename model')
				this.addXmlListTemplateData(cellParams, mxUtils.bind(this,function(xhr){
						if (xhr.readyState == 4)
						{
							this.spinner.stop();
							...
							if (xhr.status >= 200 && xhr.status <= 299)
							{
								...
								if(responseText['detail'] != undefined){
									this.handleError({message: responseText['detail'][0]})	
								}
								var xml = this.editor.graph.compress(responseText.xml);
								var entry = {xml: xml, w: bounds.width, h: bounds.height};
								if (title != null)
								{
									entry.title = responseText.name;
								}
								// console.log(images, entry, 'includeCopyOfMyDiagram')
								images.push(entry);
								var xmlModelArr = this.createLibraryDataFromImages(images)
								var resFile = ''
							  if(responseText.group_name == null && responseText.group_id == null){
							  	resFile = new StorageLibrary(this, xmlModelArr, '.scratchpad')
							  	resFile.group_id = responseText.group_id
							  }else{
							  	resFile = new StorageLibrary(this, xmlModelArr, responseText.group_name)
							  	resFile.group_id = responseText.group_id
							  }
							  contentDiv.setAttribute('groupid', responseText.group_id); 
							  // addImages(resFile, content);
							  this.loadLibrary(resFile);
								
							}
							else
							{
								this.handleError({message: '添加图元模板失败'});
							}
						}
				}));
			...
			var addSelection = mxUtils.bind(this, function(evt)
			{
				...
				if (!graph.isSelectionEmpty())
				{
					...
					bounds.group_id = evt.target.getAttribute('groupid')
			...
			contentDiv.setAttribute('groupid', file.group_id)
			...
			}), mxUtils.bind(this, function(evt)
			{
				...
				if (graph.isMouseDown && graph.panningManager != null && graph.graphHandler != null)
				{
					...
					// console.log(contentDiv, 'contentDiv ---6')
					evt.target.setAttribute('groupid', file.group_id)
					addSelection(evt);
					mxEvent.consume(evt);
				}
			}));
			...
			btn2.setAttribute('groupid', file.group_id);
			// btn2.width = '20';
			// btn2.height = '20';
			var addBtnBox = document.createElement('span');
			addBtnBox.style.width = '18px';
			addBtnBox.style.height = '13px';
			addBtnBox.style.display = 'inline-block';
			addBtnBox.style.padding = '8px';
			addBtnBox.style.textAlign = 'center';
			addBtnBox.appendChild(btn2)
			buttons.insertBefore(addBtnBox, buttons.firstChild);
			buttons.style.padding = '0px';
			mxEvent.addListener(addBtnBox, 'click', addSelection);
...
//				}));
	...
	EditorUi.prototype.showLibraryDialog = function(name, sidebar, images, file, mode)
	{
		...
		console.log(dlg, 'show editor Dida')
	 ...
	 * @param {number} dy Y-coordinate of the translation.
	 */
	EditorUi.prototype.doSaveLocalFileRequest = function(url, params, open, fn){
		var xhr = new XMLHttpRequest(); // 新建XMLHttpRequest对象
		xhr.onreadystatechange = function () { // 状态发生变化时，函数被回调
			fn(xhr)
		}
	...
	{
		// var model_type_id = this.getUrlParam('modelTypeId'); //图表ID
		var currentId = this.getUrlParam('currentId'); //图表ID
		var current_cell_Id = this.getUrlParam('cellId'); //图元ID
		var is_manager = this.getUrlParam('onFrom'); //创建，编辑来源
		var electric_station_id = this.getUrlParam('electric_station_id') //电厂id
		var current_cell_name = this.getUrlParam('cellName').replace(/%20/g, ' ');
		var creator = this.getUrlParam('un'); //操作者user name
		var isModle = this.getUrlParam('addType'); //创建模式 图元模板 PRIMITIVE 图表 GRAPH
		var name_txt = filename.split('.')[0]; //模板 图表名称
		var postOpen = 'POST'; //请求方式 
		// console.log(isModle, 'isModle')
		// var EditorUi.baseUrl_API = 'http://soc.122.204.161.218.xip.io:9023/api'
		var group_id=null;
		var aaa=[{name:"ad",group_id:"id"},{name:"ad",group_id:"id"},{name:"ad",group_id:"id"},{name:"ad",group_id:"id"},{name:"ad",group_id:"id"},]
		// console.log(currentId, 'currentId')
		if(is_manager == 'manager'){
			if(isModle == 'PRIMITIVE'){//创建，修改图元
				var xml_data = this.extractGraphModelFromHtml(data)
				if(xml_data.indexOf('<object') < 0){
					this.handleError({message: '模板内容不合规，请绑定模板属性数据后继续提交！'})	
						// this.alert('模板内容不合规，请绑定模板属性数据后继续提交！')
						return
				}else if(!group_id){
					group_id=this.handleRadio(aaa)
				}
				...
				var params = {
						...
						creator : creator,
						name: name_txt,
						modelTypeId: currentId,
						group_id: group_id
					}
				console.log(params)
				if(currentId != ''){ //修改图元
					postOpen = 'PUT'; 
					var url = EditorUi.baseUrl_API + 'mx_graph/mx_primitive/' + currentId +　'/';
				}else{
					var url = EditorUi.baseUrl_API + 'mx_graph/mx_primitive/';
				}

			}else{//创建，修改 图表
				if(current_cell_Id != ''){//创建子图
					var url = EditorUi.baseUrl_API + 'mx/create_subgraph';
				}else if(currentId == '' && current_cell_Id == ''){//创建母图
				  var url = EditorUi.baseUrl_API + 'mx/';	
				}else if(currentId != '' && current_cell_Id == ''){
					var url = EditorUi.baseUrl_API + 'mx/' + currentId + '/';
				}
				var xml_data = '<?xml version="1.0" encoding="UTF-8"?>\n' + this.extractGraphModelFromHtml(data)	
			  var params = {
					graph_id: currentId,
					attribute_id: current_cell_Id,
					attribute_name: name_txt,
					electric_station_id: electric_station_id?electric_station_id:19,
					name: name_txt,
					creator : creator,
					xml: xml_data
				}
			...
			this.handleError({message: '当前角色没有操作权限，支持管理员权限操作！'});
			return 
		}

		if(filename == '' || name_txt == '未命名表单'){
			this.handleError({message: '图表或者模板名称不合法，请确认图表名称！'});
			return
		}
		console.log(filename)
		if(isModle == 'PRIMITIVE' &&　currentId!=''){ //修改图元
			var modeParams = {
				modelTypeId: currentId
			}
			var getUrl = EditorUi.baseUrl_API + 'mx_graph/mx_primitive/get_all_used_graph/?modelTypeId=' + currentId;
			this.getXmlListTemplateData(getUrl, mxUtils.bind(this, function(xhr){
				if (xhr.readyState === 4) { // 成功完成
		        // 判断响应结果:
		        if (xhr.status === 200) {
		            // 成功，通过responseText拿到响应的文本:
		            var responseData = JSON.parse(xhr.response)
	            	var dlg = new garphModelDialog(this, responseData, creator, isModle, is_manager,mxUtils.bind(this, function(status){
						params.update_graph = status;
	            		this.doSaveLocalFileRequest(url, params, postOpen, mxUtils.bind(this, function(xhr){
										if (xhr.readyState === 4) { // 成功完成
									        // 判断响应结果:
									        if (xhr.status === 200) {
									            // 成功，通过responseText拿到响应的文本:
									            ...
									            if(responseData['detail'] != undefined){
									              ...
									              this.handleError({message: responseData['detail'][0]});
									            }else{
									            	this.alert(mxResources.get('saveSuccess'));
									            }
									            //保存后清空当前画布
															// this.editor.resetGraph();
													    // this.editor.graph.model.clear();
									            if(currentId != ''){ //current_cell_Id != '' || model_type_id != ''
							                  ...
							                  window.close();
									            ...
									            // 失败，根据响应码判断失败原因:
									            // this.handleError(xhr.response['detail'][0])
								...
								this.showDialog(dlg.container, 400, 450, true, false);
		        } else {
		            // 失败，根据响应码判断失败原因:
		            // this.handleError(xhr.response['detail'][0])
		            console.log(xhr.response, 'res')
			...
			console.log("1")
			var dlg = new garphRadioBtn(this, responseData, creator, isModle, is_manager,mxUtils.bind(this, function(status){
				params.update_graph = status;
				console.log(this)
				this.doSaveLocalFileRequest(url, params, postOpen, mxUtils.bind(this, function(xhr){
								if (xhr.readyState === 4) { // 成功完成
...
EditorUi.prototype.getUrlParam = function(paraName) {
...
　if (arrObj.length > 1) {
	...
	EditorUi.prototype.saveLocalFile = function(data, filename, mimeType, base64Encoded, format, allowBrowser, allowTab)
	{
		...
		var dlg = new CreateDialog(this, filename, mxUtils.bind(this, function(newTitle, mode)
		{
			try
			{
				...
				else if (mode == App.MODE_DEVICE || mode == 'download' || mode == 'save')
				{
					//文件保存 入库
	...
	EditorUi.prototype.saveData = function(filename, format, data, mime, base64Encoded)
	{
		...
		// var format = format
		// mxUtils.post(url,'"file_name=" + file_name, "format=" +　format', function(req){
		// 		mxUtils.alert(mxResources.get('saveSuccess'))
		// })
			...
			else
			{
				data = '#R' + encodeURIComponent((allPages) ?
					this.getFileData(true, null, null, null, null, null, null, true) :
					 mxUtils.getXml(this.editor.getGraphXml())) //this.editor.graph.compress()
			}
		}
...
//		}
	...
	EditorUi.prototype.init = function()
	{
		...
		this.getModeGroupListData();
		this.getModelLastestData(0);
	...
	EditorUi.prototype.open = function(){
		var e = this.editor
		setTimeout(function(){
			e.setGraphXml(mxUtils.parseXml(xml).documentElement)
		}, 200)
